...
class map2D(object):
    ...
    def set_particle(self, npart=1, emit=1e-6, twiss_beam=None, espr=0.0, particles=None):
        ...
        if particles is None:
            ...
            self.coor2D[0, :]= rng.normal(npart)
            ...
            self.coor2D[1, :] = rng.normal(npart)
            ...
            emit=np.sqrt(np.abs(sizex * sizexp * sizex * sizexp - cxxp * cxxp))
        ...
        if espr!=0 and self.chrom!=0:
            ...
            #same as self.linmap which is one turn map
    ...
    def statistics(self):
         ...
         return avex, avexp, sizex, sizexp, np.sqrt(np.abs(sizex*sizexp*sizex*sizexp -cxxp*cxxp))